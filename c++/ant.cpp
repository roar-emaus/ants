...
std::uniform_real_distribution<double> Ant::random_angle_distribution(-0.1,
                                                                      0.1);
...
Ant::Ant(double start_x, double start_y, int arena_size, double nest_x,
         double nest_y, std::shared_ptr<FoodSource> food_source)
    : x(start_x), y(start_y), arena_size(arena_size), nest_x(nest_x),
      nest_y(nest_y), food_source(food_source), job(Status::IN_NEST),
...
void Ant::move(Arena& arena) {
  if (job == Status::IN_NEST) {
    ...
    if (carrying_food) {
      ...
      if (food_source) {
        strength =
            10000 / distance(x, y, food_source->get_x(), food_source->get_y());
      }
      update_position();
...
bool Ant::should_follow_pheromone(Arena& arena) {
  ...
  double follow_probability =
      1.0 / (1.0 + std::exp(-Constants::SIGMOID_SCALE *
                            (pheromone_level - Constants::SIGMOID_MIDPOINT)));
...
bool Ant::exit_nest() { return normal_distribution(generator) < 0.0001; }
