...
void Arena::update_pheromones(const FoodSource& food_source) {
  double const evaporation_rate =
      food_source.is_food_source_empty()
          ? Constants::INCREASED_PHEROMONE_DISSIPATION_RATE
          : Constants::PHEROMONE_DISSIPATION_RATE;
...
void Arena::diffuse_pheromones() {
  std::vector<std::vector<double>> new_pheromone_grid(
      arena_size, std::vector<double>(arena_size, 0.0));
